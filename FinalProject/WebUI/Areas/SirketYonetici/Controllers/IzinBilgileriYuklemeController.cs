            ...
            using (ExcelPackage package = new ExcelPackage(file))
            {
                ...
                else
                {
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        ...
                        {
                            Ad = (worksheet.Cells[row, 1].Value ?? string.Empty).ToString().Trim(),
                            Soyad = (worksheet.Cells[row, 2].Value ?? string.Empty).ToString().Trim(),
                            EPostaSirket = (worksheet.Cells[row, 3].Value ?? string.Empty).ToString().Trim(),
                            YıllıkIzinHakki = (worksheet.Cells[row, 4].Value ?? string.Empty).ToString().Trim()
                        };
